...
int main(void) {
...

    {
        ...
        const Span<const u8> const_test_view_bytes = test_view.ConstBytes();
...

    {
        ...
        ASSERT(bits.ReadBits(1) == 0);
        ASSERT(bits.ReadBits(1) == 1);
        ASSERT(bits.ReadBits(1) == 0);
        ASSERT(bits.ReadBits(1) == 1);
        ASSERT(bits.ReadBits(2) == 3);
        ASSERT(bits.ReadBits(1) == 0);
        ASSERT(bits.ReadBits(1) == 1);
        ASSERT(!bits.IsOverrun());
        bits.ReadBits(1);
        ASSERT(bits.IsOverrun());
    }
    CHECK_LEAKS();

    {
        ...
        Hash::MD5String("The quick brown fox jumps over the lazy dog").Render(digest, ARRLEN(digest));
