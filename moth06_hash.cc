...
u32 Hash::FNV(BufferView view) {
    u32 result = FNV_OFFSET;
    for (usize i = 0; i < view.len; ++i) {
        result ^= (u32)view.buf[i];
        result *= FNV_PRIME;
    }
    return result;
}

u32 Hash::FNVString(const char* str) {
    u32 result = FNV_OFFSET;
...
Hash::MD5Digest Hash::MD5(BufferView view) {
    ...
    Array<u8> input = Array<u8>((((view.len + 8) / 64) + 1) * 64);

    // Copy initial buffer
    Mem::Copy(input.Buffer(), view.buf, view.len);

    // Set 1 bit
    input[view.len] = 1 << 7;

    // Write size in bits
    u64 size_bits = view.len * 8;
    Mem::Copy(&input[input.Length() - sizeof(u64)], (u8*)&size_bits, sizeof(u64));
    ...
    for (usize i = 0; i < input.Length() / 64; ++i) {
        u32 a = state_A;
        u32 b = state_B;
        u32 c = state_C;
        u32 d = state_D;
    ...
    Mem::Copy(result.m_bytes, (u8*)result32, ArrLen(result.m_bytes));

    return result;
}

Hash::MD5Digest Hash::MD5String(const char* str) {
    return Hash::MD5(BufferView((u8*)str, Str::Length(str)));
}
